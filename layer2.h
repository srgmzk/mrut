...
#define ETH_HDR_SIZE_EXCL_PAYLOAD (2*sizeof(struct mac_addr_t)+sizeof(unsigned short))

#define ETH_FCS(eth_hdr_ptr, payload_size) ((eht_hdr_ptr) + ETH_HDR_SIZE_EXCL_PAYLOAD + (payload_size))

static inline ethernet_hdr_t *
ALLOC_ETH_HDR_WITH_PAYLOAD(char *pkt, unsigned int pkt_size) {
	struct ethernet_hdr_t* ether_hdr;
